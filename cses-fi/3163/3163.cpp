...
int main() {
    ...
    ll m = vals.size();
    vector<vector<tuple<ll,ll,ll>>> as(n);
    vector<vector<tuple<ll,ll,ll>>> bs(n);
    
    forn(i,0,n) compress(vec[i]);
    
    forn(i,0,q){
        auto& [a,b,c,d]=  querries[i];
        compress(c);
        compress(d);
        if(a > 0) as[a-1].push_back({c,d,i});
        bs[b].push_back({c,d,i});
    }
    v64 resp(q);
    segtree seg(m, node());
