...
struct perseg {
	...
	static ll set(Node* d, ll& cnt, ll a, ll val, ll lp, ll p, ll l, ll r) {
		if (l == r) return d[p].val = d[lp].val + val;
		ll m = (l+r)/2;
		if (a <= m) {
...
int main() {
    ...
    ll n, q; cin >> n >> q;
    v64 vec(n), vals;
    vector<tuple<ll,ll,ll,ll>> querries(q);
    ...
    v64 t(n);
    forn(i,0,n){
        // ll val = seg.query(vec[i], vec[i], (i == 0 ? 0 : t[i-1]));
        t[i] = seg.set(vec[i],1, (i == 0 ? 0 : t[i-1]));
    }
    
    for(auto& [a,b,c,d] : querries){
        ll valb = seg.query(c,d,t[b]);
        ll vala = 0;
        if(a > 0) vala = seg.query(c,d,t[a-1]);
        debug(valb);
        debug(vala);
        cout << valb - vala << ln;
    }
