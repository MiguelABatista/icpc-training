...
void update(ll v, vector<p64>& cities){
    ll anc = v;
    ll cnt = 0;
    while(anc != -1){
        if(dist[v][cnt] <= cities[anc].first){
            cities[anc].first = dist[v][cnt];
            cities[anc].second = v;
        }
        anc = p[anc];
        cnt++;
...
ll nearest_valid(ll v, vector<p64>& cities){
    ll curr_dist = INF;
    ll resp = -1;
    ll anc = v;
    ll cnt = 0;
    while(anc != -1){
        if(cities[anc].first+dist[v][cnt] < curr_dist){
            curr_dist = cities[anc].first+dist[v][cnt];
            resp = cities[anc].second;
        }else if(cities[anc].first+dist[v][cnt] == curr_dist){
            if(resp > cities[anc].second){
                resp = cities[anc].second;
            }
        }
        anc = p[anc];
...
int main(){
    ...
    vector<p64> cities(n, {INF, -1}); 
    v64 answer(n, -1);
        
    for(ll i = n-1; i>= 0; i--){
        answer[i] = nearest_valid(i, cities);
        update(i, cities);    
    }
    answer[n-1] = n-1;

    forn(i,0,n) cout << answer[i]+1 << " ";; cout << ln;
    return 0;
}