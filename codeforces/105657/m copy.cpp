...
bool test(ll x, ll l, ll r){
    ...
    if(mdc%(p_min.first + x)!= 0) return false;

    bool b1 = test(x, l, p_min.second - 1);
    if(b1 == false) return false;
    return test(x, p_min.second + 1, r);
}
...
void solve(){
    ...
    for(ll i = 1; i*i <= mdc; i++){
        ...
        if(mdc%i != 0) continue;
        if(i > worst && test(i-worst, 0, n-1)){
            sum += (i-worst);
            count++;
        }
        
        if((i*i!= mdc) && (mdc/i) > worst){
            if(test((mdc/i)-worst, 0, n-1)){
                sum += (mdc/i)-worst;
                 count++;
            }
        }
