...
bool solve(ll n){
    ...
    if(ini != -1 && degs[ini] == 0) return false;
    if(fim != -1 && degs[fim] == 0) return false;
...
int main(){
    ...
    if(solve(n)){
        ...
        v64 path = euler_path();
        v64 resp = vl;
        for(ll x: path) resp.push_back(x);
        for(ll x: vr) resp.push_back(x);
